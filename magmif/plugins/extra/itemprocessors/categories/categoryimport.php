...
class CategoryImporter extends Magmi_ItemProcessor
{
	...
	public function initCats()
	{
		...
		/*
		 ...
		 ");
		*/
		$result=$this->selectAll("SELECT cs.store_id,csg.website_id,ea.entity_type_id, cce.path,ccev.value as name
								FROM $cs as cs 
								JOIN $csg as csg on csg.group_id=cs.group_id
								JOIN $t as cce ON cce.entity_id=csg.root_category_id
		...
		foreach($result as $row)
		{
			...
			//$rootinfo=array("path"=>$row["path"],"name"=>$row["name"],"rootarr"=>explode("/",$row["path"]));
...
//exit;		
		...
		foreach($this->_cattrinfos as $tp=>$attinfo)
		{
			...
			$sql="INSERT INTO $tb (attribute_id,store_id,entity_id,value) VALUES ".implode(",",$inserts).
			" ON DUPLICATE KEY UPDATE value=VALUES(`value`)";
