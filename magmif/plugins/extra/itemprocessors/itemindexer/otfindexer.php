...
class ItemIndexer extends Magmi_ItemProcessor
{
	...
	public function initialize($params)
	{
		...
		$this->tns=array("cpe"=>$this->tablename("catalog_product_entity"),
						 ...
						 "curw"=>$this->tablename("url_rewrite"));
	...
	public function buildCatalogCategoryProductIndex($pid)
	{
	 ...
	 /**
	 * Build url_rewrite index entry for given pid
	 * @param int $pid , product id to create index entry for
	 */
	public function buildUrlCatProdRewrite($pid,$purlk)
	{
					...
					*/
					if($pinfo["store_id"] != 0){
						if($pinfo["catid"] != 2){
							...
							$tmpArray = array();
							$tmpCatArray["category_id"] = $catid;
							$catidMetadata = serialize($tmpCatArray);
							//print_r($tmpCatArray);
							$sdata=array($pid,$storeid,$catidMetadata,"product","catalog/product/view/id/$pid/category/$catid","$curlk/$purlk",1);
			...
			if(count($vstr)>0)
			{
				...
				*/
			$sqlprodcat="INSERT IGNORE INTO {$this->tns["curw"]} (entity_id,store_id,metadata,entity_type,target_path,request_path,is_autogenerated) VALUES ".implode(",",$vstr);
				
				/*$sqlprod="INSERT INTO {$this->tns["curw"]} (entity_id,store_id,entity_type,target_path,request_path,is_autogenerated) $produrlsql ON DUPLICATE KEY UPDATE request_path=VALUES(`request_path`)";
				*/
	...
	public function builProductUrlRewrite($pid)
	{
		...
		$sql="DELETE FROM {$this->tns["curw"]} WHERE entity_id=? AND is_autogenerated=1";
		$this->delete($sql,$pid);
		//echo $purlk;
		//echo '<br>';exit;
