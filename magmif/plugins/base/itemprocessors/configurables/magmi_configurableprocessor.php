...
{	
	if(!isset($this->_configurable_attrs[$asid]))
	{
		...
		/*
			 SELECT `main_table`.`attribute_code`, `cea`.apply_to FROM `eav_attribute` AS `main_table`
 INNER JOIN `catalog_eav_attribute` AS `cea` ON cea.attribute_id = main_table.attribute_id 
 
 WHERE (main_table.entity_type_id = 4) AND (`frontend_input` = 'select') AND (`is_user_defined` = '1') AND (`is_global` = '1')
		
		
		SELECT ea.attribute_code FROM `eav_attribute` as ea
		JOIN eav_entity_type as eet ON eet.entity_type_id=ea.entity_type_id AND eet.entity_type_id=4
		JOIN eav_attribute_set as eas ON eas.entity_type_id=eet.entity_type_id AND eas.attribute_set_id=10
		JOIN eav_entity_attribute as eea ON eea.attribute_id=ea.attribute_id JOIN catalog_eav_attribute as cea ON cea.attribute_id=ea.attribute_id AND cea.is_global=1 AND cea.`apply_to` IS NULL WHERE ea.is_user_defined=1 AND ea.frontend_input= 'select'
		...
		*/
		...
		$sql.=" JOIN $cea as cea ON cea.attribute_id=ea.attribute_id AND cea.is_global=1 AND cea.`apply_to` IS NULL";
		$cond="ea.is_user_defined=1 AND ea.frontend_input= 'select'";
		$sql.=" WHERE $cond GROUP by ea.attribute_id";
			

		/*	
		echo '<br>';
		echo $this->getProductEntityType();
		echo '<br>';
	...
	//echo 111;
	//print_r($this->_configurable_attrs[$asid]);exit;	
