...
class Magmi_ProductImportEngine extends Magmi_Engine {
    ...
    public $_category_product_position;
    ...
    public function getAttributeSetId($asname) {
        ...
        if (!isset($this->attribute_sets[$asname])) {
            ...
            $asid = $this->selectone(
                    "SELECT attribute_set_id FROM $tname WHERE attribute_set_name=? AND entity_type_id=?", array($asname, $this->prod_etype), 'attribute_set_id');
    ...
    public function getProductIds($sku) {
        ...
        $result = $this->selectAll(
                "SELECT sku,entity_id as pid,attribute_set_id as asid FROM $tname WHERE sku=?", $sku);
        if (count($result) > 0) {
            return $result[0];
        } else {
            return false;
    ...
    public function createAttributes($pid, &$item, $attmap, $isnew) {
        ...
        foreach ($attmap as $tp => $a) {
            ...
            if (!empty($deletes)) { 
				...
				/*
				select catalog_product_entity_varchar.* FROM catalog_product_entity_varchar JOIN eav_attribute ON catalog_product_entity_varchar.attribute_id = eav_attribute.attribute_id WHERE catalog_product_entity_varchar.attribute_id IN (84,85,86) AND store_id IN (0) AND entity_id = 2059
				*/				
				/* thumbnail and small image is fixed - By Ajai*/
    ...
    public function updateStock($pid, $item, $isnew) {
        ...
        */
		...
		if(isset($wsids[0]) && $wsids[0] != ''){
			$wsid = $wsids[0];
		} else {
			$wsid = 1; // website id for default 
		}
		$sql = "INSERT IGNORE INTO `$csit` (product_id,stock_id,website_id) VALUES (?,?,?)";
        $this->insert($sql, array($pid, $stock_id, $wsid));
...
//            $this->setDebug(1, "alex.txt");
//            $this->logdebug("alexandar");
//            if ($debug == true) {
//                echo "alexandar";
//            }
            //Here we are checking for position already created are not
            $position_avilability = $this->checkPosition($ccpt, $cdata[$i], $pid);
            
//          $this->logdebug("test_positon" . $position_avilability . "data test");
            $inserts[] = "(?,?,?)";
            $data[] = $cdata[$i];
            $data[] = $pid;
            if (isset($position_avilability)) {
                $data[] = $position_avilability;
            } else {
                $data[] = $cpos[$i];
            }
        }
        ...
        if (count($inserts) > 0) {
			...
			//print_r($data);
			$aj = 0;
			$deleteArray = array();$insertArray = array();$x = array();
			
			foreach ($rcat7 as $rcat) {
				$x[] = $rcat['category_id'];
				$x[] = $rcat['product_id'];
				$x[] = $rcat['position'];
			}
			
			for ($m = 0; $m < count($data); $m=$m+3) {
				$insertArray[] = $data["$m"];
			}
			for ($m = 0; $m < count($x); $m=$m+3) {
				$deleteArray[] = $x["$m"];
			}
			
			$difrenceArray = array_diff($deleteArray, $insertArray);
			if(count($difrenceArray) >0 ){
				
				$difrenceIdlist = implode(",", $difrenceArray);
				$difrenceIdlist2 = explode(",", $difrenceIdlist);
				$dd[] = $pid;
				//$dd[] = $difrenceIdlist;
				
				$sqlForDelete="DELETE $ccpt.*
				FROM $ccpt
				JOIN $cce ON $cce.entity_id=$ccpt.category_id
				WHERE product_id=? and category_id IN (" . $difrenceIdlist . ") ";
				$this->delete($sqlForDelete, $dd);
				//$ss = $this->selectAll($sqlForDelete,$ddata);
				
				//print_r($ss);
				/*
				
				select catalog_category_product.*
				FROM catalog_category_product
				JOIN catalog_category_entity ON catalog_category_entity.entity_id=catalog_category_product.category_id
				WHERE product_id=2059 and and category_id IN (23) 
				...
				FROM catalog_category_product
				JOIN catalog_category_entity ON catalog_category_entity.entity_id=catalog_category_product.category_id
				WHERE product_id= 2059 and category_id in(23,24)
				*/
			}
			//print_r($difrenceArray);
			//-------------------------------------------------------
            $this->insert($sql, $data);
            unset($data);
			unset($dd);
    ...
    public function checkPosition($ccpt, $catid, $pid) {
        $data = $this->selectone("SELECT position from $ccpt WHERE category_id=? AND product_id=?", array($catid, $pid), 'position');
